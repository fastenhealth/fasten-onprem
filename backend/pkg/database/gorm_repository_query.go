...
func (gr *GormRepository) GetLastUpdatedTimestamp(ctx context.Context) (*time.Time, error) {
	...
	for _, resourceType := range databaseModel.GetAllowedResourceTypes() {
		...
		tableName := strcase.ToSnake("Fhir" + resourceType)
		...
		allTimestamps = append(allTimestamps, result.UpdatedAt)
	}
	...
	sort.Slice(allTimestamps, func(i, j int) bool {
		return allTimestamps[i].After(allTimestamps[j])
	})

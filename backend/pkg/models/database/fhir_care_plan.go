...
func (s *FhirCarePlan) PopulateAndExtractSearchParameters(rawResource json.RawMessage) error {
	...
	instantiatesUriResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'CarePlan.instantiatesUri')[0])")
	...
	activityDateResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'CarePlan.activity.detail.scheduled')[0])")

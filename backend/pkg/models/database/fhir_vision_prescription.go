...
func (s *FhirVisionPrescription) SetOriginBase(originBase models.OriginBase) {
...
func (s *FhirVisionPrescription) PopulateAndExtractSearchParameters(rawResource json.RawMessage) error {
	...
	datewrittenResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'VisionPrescription.dateWritten')[0])")
	...
	// extracting Status
	statusResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'VisionPrescription.status'))")

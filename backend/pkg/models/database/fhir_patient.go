...
func (s *FhirPatient) PopulateAndExtractSearchParameters(rawResource json.RawMessage) error {
	...
	deathDateResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, '(Patient.deceaseddateTime)')[0])")
	...
	// extracting Given
	givenResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Patient.name.given | Practitioner.name.given')[0])")
	...
	// extracting Name
	nameResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Patient.name')[0])")
	...
	// extracting Family
	familyResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Patient.name.family | Practitioner.name.family')[0])")

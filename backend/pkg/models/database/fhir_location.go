...
func (s *FhirLocation) PopulateAndExtractSearchParameters(rawResource json.RawMessage) error {
	...
	addressStateResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Location.address.state')[0])")
	...
	addressCountryResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Location.address.country')[0])")
	...
	addressCityResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Location.address.city')[0])")
	...
	// extracting Address
	addressResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Location.address')[0])")
	...
	// extracting Name
	nameResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Location.name | Location.alias')[0])")
	if err == nil && nameResult.String() != "undefined" {
		s.Name = nameResult.String()
	}
	// extracting AddressPostalcode
	addressPostalcodeResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'Location.address.postalCode')[0])")

...
func (s *FhirMedicationRequest) PopulateAndExtractSearchParameters(rawResource json.RawMessage) error {
	...
	// extracting Date
	dateResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'MedicationRequest.dosageInstruction.timing.event')[0])")
	...
	authoredonResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'MedicationRequest.authoredOn')[0])")

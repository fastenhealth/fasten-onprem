...
func (s *FhirServiceRequest) PopulateAndExtractSearchParameters(rawResource json.RawMessage) error {
	...
	// extracting Authored
	authoredResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'ServiceRequest.authoredOn')[0])")
	...
	occurrenceResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'ServiceRequest.occurrence')[0])")
	...
	instantiatesUriResult, err := vm.RunString("JSON.stringify(window.fhirpath.evaluate(fhirResource, 'ServiceRequest.instantiatesUri')[0])")
